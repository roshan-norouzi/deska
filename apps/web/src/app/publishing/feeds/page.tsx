                  ...
                  {visibleFeeds.map((feed) => {
                    ...
                    return (
                      <tr key={feed.id} className="transition hover:bg-slate-50/80">
                        ...
                        <td className="px-5 py-4"><div className="flex items-center gap-1">{feed.purpose === 'news-room' && <Button size="sm" variant="ghost" title="پایش فید اتاق خبر" isLoading={busy === `fetch-${feed.id}`} onClick={() => run(`fetch-${feed.id}`, async () => { await apiFetch(`/publishing/feeds/${feed.id}/fetch`, { method: 'POST' }); setNotice({ type: 'success', text: `پایش «${feed.name}» انجام شد و خبرهای جدید در صف آماده‌سازی قرار گرفتند.` }); await refetch(); })}><RefreshCw className="h-4 w-4" /></Button>}<Button size="sm" variant="ghost" title="ویرایش" onClick={() => openEdit(feed)}><Pencil className="h-4 w-4" /></Button><Button size="sm" variant="ghost" title={feed.enabled ? 'توقف' : 'فعال‌سازی'} onClick={() => run(`toggle-${feed.id}`, async () => { await apiFetch(`/publishing/feeds/${feed.id}/toggle`, { method: 'POST' }); await refetch(); })}><Power className={cn('h-4 w-4', feed.enabled ? 'text-emerald-600' : 'text-slate-400')} /></Button><Button size="sm" variant="ghost" title="حذف" className="text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => { if (window.confirm(`فید «${feed.name}» حذف شود؟ مطالب منتشرشده حفظ می‌شوند.`)) run(`delete-${feed.id}`, async () => { await apiFetch(`/publishing/feeds/${feed.id}`, { method: 'DELETE' }); setNotice({ type: 'success', text: 'فید حذف شد.' }); await refetch(); }); }}><Trash2 className="h-4 w-4" /></Button></div></td>
                      </tr>
                    );
                  })}
                </tbody>
